...
export default function BookingForm({
  ...
  // Book CTAs elsewhere on the page preselect the flight type here.
  useEffect(() => {
    const onBook = (event: Event) => {
      const service = (event as CustomEvent<string>).detail;
      if ((FLIGHT_TYPES as readonly string[]).includes(service)) {
        setType(service);
      }
    };
    window.addEventListener("lukla:book", onBook);
    return () => window.removeEventListener("lukla:book", onBook);
  }, []);
  ...
  if (submitted) {
    return (
      <div className="rounded-[14px] bg-white p-6 text-center text-[#12233F] shadow-[0_22px_60px_rgba(5,31,73,.45)] sm:p-8">
        ...
        <h3 className="font-fraunces leading-[1.12] text-xl font-extrabold text-[#051F49]">
          Request sent.
        </h3>
  ...
  return (
    ...
    >
      {heading ? (
        <>
          <h2 className="font-fraunces leading-[1.12] text-[1.15rem] font-extrabold text-[#051F49]">
      ...
      <Field label="What do you want to fly?" htmlFor={id("type")}>
        <select
          ...
          className="w-full rounded-[9px] border-[1.5px] border-[#D7E2F2] bg-white px-3.5 py-3 font-geist text-[15.5px] text-[#12233F] focus:border-[#0C4396] focus:outline focus:outline-2 focus:outline-offset-1 focus:outline-[#0C4396]"
        >
      ...
      <Field label="Phone / WhatsApp number" htmlFor={id("phone")} error={errors.phone}>
        <div className="grid grid-cols-[minmax(112px,38%)_1fr] gap-2.5">
          <select
            ...
            className="rounded-[9px] border-[1.5px] border-[#D7E2F2] bg-white px-2 py-3 font-geist text-[14.5px] text-[#12233F] focus:border-[#0C4396] focus:outline focus:outline-2 focus:outline-offset-1 focus:outline-[#0C4396]"
          >
...
function inputClass(invalid: boolean) {
  return `w-full rounded-[9px] border-[1.5px] bg-white px-3.5 py-3 font-geist text-[15.5px] text-[#12233F] focus:outline focus:outline-2 focus:outline-offset-1 focus:outline-[#0C4396] ${
