...
function RouteCard({ title, meta, href }) {
  return (
    <Link
      href={href}
      className=" group flex h-full min-h-[178px] flex-col border-[0.67px] border-[#003366]/15 bg-white p-5 transition-colors duration-300 hover:border-[#F2B632] sm:p-6 "
    >
...
export default function EverestRelatedRoutes() {
  return (
    <section id="related-routes" className="scroll-mt-[123px] overflow-hidden">
      {/* Related routes */}
            ...
            <div className="mt-4 flex flex-col items-start gap-2">
              {alsoSeeLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className=" inline-flex items-center gap-1 font-manrope text-[14px] font-normal leading-[16px] tracking-[0px] text-[#1686D9] transition-colors hover:text-[#003366] "
                >
                  {link.label}
      ...
      <div className=" border-t-[0.67px] border-t-[#003366]/15 bg-[#EDF4FA] ">
        <div className=" mx-auto w-full max-w-[1440px] px-5 py-16 sm:px-8 lg:px-12 lg:py-20 xl:px-[125px] ">
          <p className=" font-manrope text-[11px] font-bold uppercase leading-[14px] tracking-[1.98px] text-[#B87800] ">
            , Why Mountain Helicopters
          </p>

          <h2 className=" mt-4 w-full max-w-[1100px] font-fraunces text-[36px] font-semibold leading-[42px] tracking-[-0.72px] text-[#003366] sm:text-[51.2px] sm:leading-[49px] lg:text-[49px] lg:leading-[56px] lg:tracking-[-1.02px] ">
